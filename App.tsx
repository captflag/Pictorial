...
const App: React.FC = () => {
  ...
  const prevLevelRef = useRef(Math.floor(xp / 1000) + 1);
  ...
  const NavItem = ({ mode, icon: Icon, label }: { mode: ViewMode, icon: any, label: string }) => (
    <button
      ...
      className={`group flex items-center gap-3 px-4 py-3 rounded-xl text-left font-medium transition-all duration-300 ${viewMode === mode
  ...
  return (
    <div className="min-h-screen font-sans selection:bg-brand-100 selection:text-brand-800">
          ...
          <nav className="hidden lg:flex items-center gap-1 bg-slate-100/50 p-1 rounded-full border border-slate-200/50">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-semibold transition-all duration-300 ${viewMode === item.m
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative" ref={menuRef}>
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all ${isSubjectMenuOpen ? 'bg-brand-600 text-white shadow-lg shadow-brand-200' : 'glass-button text-slate-600'
                  }`}
              >
                <School size={16} />
                  ...
                  <div className="w-1/4 p-2 space-y-1 bg-slate-50/50 rounded-l-2xl">
                    ...
                    {CBSE_CLASSES.map((cls) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium flex justify-between items-center transition-all ${activeClass === cls.id ? 'bg-white shadow-sm text-brand-600 ring-1 ring-slate-100' : 'text-slate-500 hover:bg-slate-100'
                          }`}
                      >
                        {cls.label}
                    ...
                    <div className="space-y-1 h-[300px] overflow-y-auto custom-scrollbar">
                      {GET_SUBJECTS(activeClass).map((subject) => (
                        <button
                          ...
                          className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-all ${activeSubject === subject ? 'bg-brand-50 text-brand-700' : 'text-slate-600 hover:bg-slate-50'
                            }`}
                        >
                          {subject}
                        </button>
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-1 pr-3 py-1 rounded-full transition-all border ${viewMode === ViewMode.DASHBOARD
      ...
      {/* Main Content */}
      <main id="main-content" className="pt-28 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen" role="main">
